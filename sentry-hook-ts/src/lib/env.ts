/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { dlv } from "./dlv.js";

import * as z from "zod";
import { SDKOptions } from "./config.js";

export interface Env {
  SPEAKEASY_API_KEY?: string | undefined;
  SPEAKEASY_WORKSPACE_IDENTIFIER?: string | undefined;
  SPEAKEASY_BEARER?: string | undefined;

  /**
   * Sets the workspaceId parameter for all supported operations
   */
  SPEAKEASY_WORKSPACE_ID?: string | undefined;

  SPEAKEASY_DEBUG?: boolean | undefined;
}

export const envSchema: z.ZodType<Env, z.ZodTypeDef, unknown> = z.object({
  SPEAKEASY_API_KEY: z.string().optional(),
  SPEAKEASY_WORKSPACE_IDENTIFIER: z.string().optional(),
  SPEAKEASY_BEARER: z.string().optional(),

  SPEAKEASY_WORKSPACE_ID: z.string().optional(),

  SPEAKEASY_DEBUG: z.coerce.boolean().optional(),
});

let envMemo: Env | undefined = undefined;
/**
 * Reads and validates environment variables.
 */
export function env(): Env {
  if (envMemo) {
    return envMemo;
  }

  envMemo = envSchema.parse(
    dlv(globalThis, "process.env") ?? dlv(globalThis, "Deno.env") ?? {},
  );
  return envMemo;
}

/**
 * Clears the cached env object. Useful for testing with a fresh environment.
 */
export function resetEnv() {
  envMemo = undefined;
}

/**
 * Populates global parameters with environment variables.
 */
export function fillGlobals(options: SDKOptions): SDKOptions {
  const clone = { ...options };

  const envVars = env();

  if (typeof envVars.SPEAKEASY_WORKSPACE_ID !== "undefined") {
    clone.workspaceId ??= envVars.SPEAKEASY_WORKSPACE_ID;
  }

  return clone;
}
