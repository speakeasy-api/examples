...
Code Regions_ to add a method to the SDK client.

The example demonstrates how to handle a specific use case: when a user needs
their operation's `requestBody` to be a union discriminated by a path parameter.

Key implementation details:

- `typescript.enableCustomCodeRegions` in `gen.yaml` must be set to `true`.
- `src/funcs/createPet.ts` contains a custom user-defined function that matches
  the behavior of other standalone functions in the folder.
- `src/sdk/sdk.ts` implements `Custom Code Regions` to add a method using the
  `createPet` function. These regions are marked by comments like `#region
  sdk-class-body`.

# petstore

Developer-friendly & type-safe Typescript SDK specifically catered to leverage _petstore_ API.
...
- [createPetCanine](docs/sdks/petstore/README.md#createpetcanine)
- [createPetAvian](docs/sdks/petstore/README.md#createpetavian)
...
async function run() {
  let result;
  try {
    result = await petStore.createPetCanine({
      id: "<id>",
      breed: "<value>",
      length: 5115.23,
    });
