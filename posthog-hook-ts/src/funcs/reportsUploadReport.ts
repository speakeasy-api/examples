...
export function reportsUploadReport(
  ...
  request: operations.UploadReportRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.UploadedReport,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.UploadReportRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json("2XX", operations.UploadedReport$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
